...
const PlayoffFixtureNavigator: React.FC<PlayoffFixtureNavigatorProps> = ({ rounds }) => {
  const [selectedPhase, setSelectedPhase] = useState<number>(rounds[rounds.length - 1].roundNumber);
  const currentPhase = rounds.find((fase: Round) => fase.roundNumber === selectedPhase);
  ...
  return (
    <>
      <Box className="flex w-full p-4 justify-between items-center">
        ...
        <Typography className="font-bold">{getFaseLabel(selectedPhase)}</Typography>
        <IconButton
      ...
      >
        {currentPhase?.matchesPlayoff.map((partido, index) => (
          <PlayoffRoundsFixture partido={partido} index={index} />
        ))}
      </Box>
    </>
  );
