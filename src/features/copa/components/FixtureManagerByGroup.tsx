...
import { Grupo } from './FaseGruposGenerator';
...
interface FixtureManagerByGroupProps {
  grupo: Grupo;
}

const FixtureManagerByGroup: FC<FixtureManagerByGroupProps> = ({ grupo }) => {
  ...
  const handleAutocompletar = () => {
    ...
    const nuevasFechas = Array.from({ length: cantidadFechas }, (_, fechaIndex) => {
      ...
      const partidos = generarPartidos(grupo.equipos, partidosPrevios, equiposUsados);
      return {
        id: fechaIndex + 1,
  ...
  return (
    <Card className="mt-6">
      <Box className="w-full flex justify-end items-center p-5">
        <Button variant="contained" color="primary" onClick={handleAutocompletar}>
...
export default FixtureManagerByGroup;
